plan_nk_brms <- function(model, n = NULL, k = NULL, width = .35, post_q = .8, 
                         conf_int = .95, tvar = 1){
  ...
  if(hasArg(k)){
    if(min_k > k){
      warning("A minimum of ", round(min_k), " clusters is required for the desired confidence interval width. ", 
              "Increase k or h_width.")
    } else {
      # Plot CI width vs n for varying k
      k1 <- post_qs_brms(model, n = 1:500, k = min_k_int, post_q = post_q)$ci_width_q
      k2 <- post_qs_brms(model, n = 1:500, k = ceiling(min_k_int*1.5), post_q = post_q)$ci_width_q
      k3 <- post_qs_brms(model, n = 1:500, k = min_k_int*2, post_q = post_q)$ci_width_q
      df1 <- data.frame(n = rep(1:500, 3), 
                        se = c(k1, k2, k3), 
                        k = rep(c("k1", "k2", "k3"), each = 500))
      plot1 <- ggplot(df1, aes(x=n, y=se, col=k)) + 
        geom_line() +
        labs(title = "C.I. Width against n for Varying k's", x = "n", y = "C.I. Width") +
      ...
      credq50 <- post_qs_brms(model, n = 1:500, k = k, post_q = .5)$ci_width_q
      credq80 <- post_qs_brms(model, n = 1:500, k = k, post_q = .8)$ci_width_q
      credq95 <- post_qs_brms(model, n = 1:500, k = k, post_q = .95)$ci_width_q
      ...
      plot2 <- ggplot(df2, aes(x=n, y=credq, col=conf)) + 
        ...
        labs(x = "n", y = "C.I. Width", 
             title = paste0("For k = ", ceiling(k), ", C.I. Width against n for Varying Credible Quantiles")) +
    ...
    # Plot CI width vs n for varying k
    k1 <- post_qs_brms(model, n = 1:500, k = ceiling(k/1.5), post_q = post_q)$ci_width_q
    k2 <- post_qs_brms(model, n = 1:500, k = k, post_q = post_q)$ci_width_q
    k3 <- post_qs_brms(model, n = 1:500, k = ceiling(k*1.5), post_q = post_q)$ci_width_q
    df1 <- data.frame(n = rep(1:500, 3), 
                      se = c(k1, k2, k3), 
                      k = rep(c("k1", "k2", "k3"), each = 500))
    plot1 <- ggplot(df1, aes(x=n, y=se, col=k)) + 
